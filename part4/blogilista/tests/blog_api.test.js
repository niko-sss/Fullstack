...
test("test if id is named id and not _id", async () => {
  ...
  const isTrue = keysOfBlog.includes("id");

  assert.strictEqual(isTrue, true);
});

test("test if POST is succesful", async () => {
...
test("test if non-inserted likes amount in POST returns 0 likes", async () => {
  ...
  const returnedBlog = await response.body.filter(
    (blog) => blog.url === "www.asdfasdfjfjdjfjdsfjddddd.com/blogs"
  );
